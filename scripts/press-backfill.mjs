...
const SYSTEM = `Extract structured facts from Indian IPO coverage. STRICT JSON only: {"financials":[{"fy":"FY24","revenueCr":n,"patCr":n,"roe":n,"roce":n,"de":n,"cfoCr":n}],"peers":[{"name":s,"pe":n,"pb":n,"roe":n}],"risks":[s],"objectsOfIssue":[s],"promoterPre":n,"promoterPost":n,"freshIssuePct":n,"registrar":s,"leadManagers":[s],"about":s,"lotSize":n,"issueSizeCr":n,"subscriptionTotal":n,"qib":n,"nii":n,"retail":n}. null for missing. Numbers only.`;
...
for (const r of rows) {
  ...
  const needs = (!filingGrade && (d.financials?.length ?? 0) < 3) || !d.about || !d.registrar || !(d.leadManagers||[]).length || !(d.objectsOfIssue||[]).length || !d.lotSize || !(d.risks||[]).length || !(d.peers||[]).length || (d.subscription.total === 0 && d.status !== "upcoming");
  ...
  const tq = await fetch("https://api.tavily.com/search", {
    ...
    body: JSON.stringify({ api_key: process.env.TAVILY_API_KEY, query: `${d.company} IPO financials revenue PAT lot size objects of issue lead manager registrar risks subscription`, search_depth: "advanced", max_results: 8, include_answer: true }),
  });
  spent++;
